...
static int serp_init(void) {
    ...
    if (ECHO_MAJOR) { //Se estiver definido 
        ...
        dev = MKDEV(ECHO_MAJOR,baseMinor);
        result = register_chrdev_region(dev,serpMinors,"serp");
    } else {
        printk(KERN_INFO "%-10sMajor is dynamically set \n","INFO:");
        result = alloc_chrdev_region(&dev, baseMinor, serpMinors,"serp");
...
int serp_open(struct inode *inodep, struct file *filep) {
    printk(KERN_ALERT "%-10sserp_open \n\n","OPEN:");
...
int serp_release(struct inode *inodep, struct file *filep) {
    printk(KERN_ALERT "%-10sserp_release \n\n","RELEASE:");
    return 0;
}
...
ssize_t serp_write(struct file *filep, const char __user *buff, size_t count, loff_t *offp){
  ...
  printk(KERN_ALERT "%-10sserp_write() \n\n","WRITE:");
  ...
  if(!echoKernelBuf) return -ENOMEM; //Not enough space
