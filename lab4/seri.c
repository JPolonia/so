...
static int seri_init(void) {
    ...
    unsigned char ier = 0; //isto é o byte para o registo de controlo de interrupçoes

    printk(KERN_ALERT "Seri, Hello World\n");
    ...
    //Activar interrupções actuando sobre o registo IER - Interrupt Enable Register
    //Será que era necessário verificar o estado/valor deste registo antes de actuar sobre ele?
    ier = UART_IER_RDI | UART_IER_RLSI | UART_IER_THRI;
    outb(ier, baseaddr + UART_IER);
    
    //FIFO
   // outb(UART_FCR_ENABLE_FIFO, baseaddr + UART_FCR);


    /* Alocar o Device e o seu Major */
    ret = alloc_chrdev_region(&dev, baseMinor, seriMinors, "seri");
    ...
    if (ret < 0) {
        ...
        /* Aloca espaço no kernel para armazenar a estrutura char device e limpa */
        seriDevs = kzalloc(seriMinors * sizeof (struct seri_devices), GFP_KERNEL);


        /* Procedimentos de inicialização e configuração dos vários dispositivos que usam o device = = dispositivos são tantos quantos os minors*/
        for (i = 0; i < seriMinors; i++) {

            /*Inicialização do device nº i que apresenta as operações da estrutura FOPS*/
            ...
            ret = 0;
            ret = cdev_add(&seriDevs[i].seriCharDev, MKDEV(major, i), 1);
            if (ret < 0) {
        ...
        /* Configurar o interrupt */
        ret = 0;
        ret = request_irq(4, interrupt_handler, 0, "seri", seriDevs->dev_irq_id);

        if (ret)
            printk(KERN_INFO "ERRO a configurar interrupt \n");
